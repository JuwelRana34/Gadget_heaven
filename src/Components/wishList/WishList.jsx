...
function Wishlist({ data }) {
  const { product_title, product_image, price, Specification, product_id } =
    data;

  const { total, datas, AddtoCart, wishlist, setWishlist, setAddtoCart } =
    useContext(dataContext);

  const handelWishDelete = (id) => {
    const afterdeleteWishlist = wishlist.filter(
  ...
  const handelAddToCart = (cart) => {
    ...
    if (total + product.price <= 1000) {
      setAddtoCart([...AddtoCart, product]);
      localStorage.setItem(
        "AddtoCart",
        JSON.stringify([...AddtoCart, product])
      );

      const updateWishList = wishlist.filter(
        (list) => list.product_id !== cart
      );
      ...
      toast.error("Total cart price exceed the limit of $1000");
      return;
    }
  };
      ...
      <div className=" md:flex py-3 px-4 bg-slate-100 my-4 rounded-xl">
        ...
        <div className="flex  md:w-[80%] gap-2 mt-3  justify-between items-center ">
          <div className=" space-y-1">
            ...
            <br />
            <button
              onClick={() => handelAddToCart(product_id)}
              className="bg-purple-600 py-1 px-3 rounded-full text-white font-semibold"
            >
              {" "}
              Add to Card
            </button>
          ...
          <div>
            <button onClick={() => handelWishDelete(product_id)}>
              <HiOutlineX
                className={` text-red-500 text-4xl border border-red-400 rounded-full p-2 hover:bg-red-100`}
              />
            </button>
          </div>
        </div>
